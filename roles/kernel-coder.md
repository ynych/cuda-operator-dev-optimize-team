...
**精确、可构建**：按设计选择 **链接 CUTLASS/cuBLAS/cuDNN/CUB** 或薄 `__global__`；构建系统（CMake `FetchContent`/子模块、与 PyTorch extension 对齐）参考设计中的 **vLLM / TensorRT-LLM** 等路径。严格遵循设计中的 API 与 tiling/workspace；若无法实现，退回设计而非擅自改蓝图。
